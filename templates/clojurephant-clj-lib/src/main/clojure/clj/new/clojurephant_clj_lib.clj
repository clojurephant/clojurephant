(ns clj.new.clojurephant-clj-lib
  (:require [clj.new.templates :as t]))

(defn clojurephant-clj-lib
  [name & args]
  (let [features (into #{} args)
        render (t/renderer "clojurephant-clj-lib")
        raw (t/raw-resourcer "clojurephant-clj-lib")
    ...
    (println "Generating a project called"
             (:name data)
             "based on the clojurephant-clj-lib template.")
