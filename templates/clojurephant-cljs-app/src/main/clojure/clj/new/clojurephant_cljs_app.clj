(ns clj.new.clojurephant-cljs-app
  (:require [clj.new.templates :as t]))

(defn clojurephant-cljs-app
  [name & args]
  (let [features (into #{} args)
        render (t/renderer "clojurephant-cljs-app")
        raw (t/raw-resourcer "clojurephant-cljs-app")
    ...
    (println "Generating a project called"
             (:name data)
             "based on the clojurephant-cljs-app template.")
    ...
    (println "  3) Enter \"(require '[dev.clojurephant.tools.figwheel :as fw])\" in the REPL.")
